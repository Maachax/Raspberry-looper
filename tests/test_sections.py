...
def test_get_state_includes_sections_block():
    ...
    assert state['sections']['list'] == [{'id': section['id'], 'name': '', 'loop_ids': [0], 'fx_overrides': {}}]
...
def test_sections_from_meta_reads_sections_when_present():
    ...
    assert sections == [{'id': 1, 'name': '', 'loop_ids': [0, 2], 'fx_overrides': {}}, {'id': 5, 'name': '', 'loop_ids': [], 'fx_overrides': {}}]
    assert next_id == 6
...
def test_sections_from_meta_migrates_old_scenes():
    ...
    sections, next_id = WebLooper._sections_from_meta(meta)
    assert sections == [{'id': 1, 'name': '', 'loop_ids': [0], 'fx_overrides': {}}, {'id': 2, 'name': '', 'loop_ids': [0, 1], 'fx_overrides': {}}]
    assert next_id == 3
...
def test_sections_from_meta_tolerates_null_and_missing_fields():
    ...
    assert sections == [{'id': 3, 'name': '', 'loop_ids': [], 'fx_overrides': {}}]
    assert next_id == 4
...
def test_sections_from_meta_skips_non_dict_scene_entries():
    ...
    sections, next_id = WebLooper._sections_from_meta(meta)
    assert sections == [{'id': 1, 'name': '', 'loop_ids': [0], 'fx_overrides': {}}]
    assert next_id == 2
